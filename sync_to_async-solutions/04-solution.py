...
async def load_repository_async(owner, repo):
    return await asyncio.to_thread(load_repository, owner, repo)
...
async def main():
    ...
    for owner, repo in rows:
        line = asyncio.create_task(load_repository_async(owner, repo))
        output.append(line)
    for line in await asyncio.gather(*output):
        print(line)
